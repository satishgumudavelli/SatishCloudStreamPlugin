...
/**
 * A pure-TMDB catalog (see TmdbScraper.kt) resolved through the exact same fleet of
 * tmdbId-driven stream sources VidboxProvider uses (TmdbExtractor.kt is a copy of
 * VidboxExtractor.kt) - useful precisely because it doesn't depend on any one catalog site's own
 * ID namespace or curation, unlike VidboxProvider (vidbox.vc's own site) or CinemaOsProvider
 * (cinemaos.tech/.in/.live's own site).
 */
class TmdbProvider : MainAPI() {
    override var mainUrl = tmdbMainUrl
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val result = TmdbScraper.browse(request.data, page)
    ...
    override suspend fun search(query: String): List<SearchResponse> =
        TmdbScraper.searchMulti(query).results.mapNotNull { it.toSearchResponse(null) }
    ...
    private fun JSONObject.toSearchResponse(defaultType: String?): SearchResponse? {
        ...
        return if (mediaType == "tv") {
            newTvSeriesSearchResponse(title, "$tmdbMainUrl/tv/$id", TvType.TvSeries) {
                this.posterUrl = poster
                this.score = score
            }
        } else {
            newMovieSearchResponse(title, "$tmdbMainUrl/movie/$id", TvType.Movie) {
                this.posterUrl = poster
                this.score = score
            }
        }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (url.contains("/tv/")) {
            val tv = TmdbScraper.tvDetail(id) ?: return null
            ...
            val episodes = tv.optJSONArray("seasons")?.let { seasons ->
                (0 until seasons.length()).mapNotNull { i ->
                    ...
                    TmdbScraper.seasonEpisodes(id, seasonNumber).map { eps ->
                        newEpisode(
                            VidLinkData(
                                id = id,
                                season = seasonNumber,
            ...
            val movie = TmdbScraper.movieDetail(id) ?: return null
